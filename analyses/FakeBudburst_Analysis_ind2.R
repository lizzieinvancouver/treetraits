...
# Make sure predictors start at zero!
fake$warm = as.numeric(fake$warm) - 1
fake$photo = as.numeric(fake$photo) - 1
...
############ !
spint <- sf[grep("^a_sp\\[", rownames(sf)),'mean'] # Now all good
...
for(i in 1:nsp){ # loop over species, as these are the random effect modeled. 
  ...
  coeff <- data.frame(
      ...
      sf[rownames(sf) %in% paste("b_warm_sp[", i, "]", sep = ""),'mean'],
      sf[rownames(sf) %in% paste("b_photo_sp[", i, "]", sep = ""),'mean']
      )
